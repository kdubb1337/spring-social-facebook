...
public class GroupTemplateTest extends AbstractFacebookApiTest {
	...
	public void getGroup() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "213106022036379"))
	...
	public void getMembers() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "213106022036379/members"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void getMemberships_currentUser() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "me/groups"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void getMemberships_specificUser() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "12345678/groups"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
			.andRespond(withSuccess(jsonResource("group-memberships"), MediaType.APPLICATION_JSON));
	...
	@Test
	public void search() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "search?offset=0&limit=25&q=Spring+User+Group&type=group&fields=owner%2Cname%2Cdescription%2Cprivacy%2Cicon%2Cupdated_time%2Cemail"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
