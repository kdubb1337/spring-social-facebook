...
public class FqlTemplateTest extends AbstractFacebookApiTest {
	...
	public void query_basic() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "fql?q=SELECT+uid%2C+status_id%2C+message%2C+time%2C+source+FROM+status+WHERE+uid%3Dme%28%29"))
	...
	public void query_basicWithFloat() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "fql?q=SELECT+vid%2C+title%2C+length+FROM+video+WHERE+owner%3Dme%28%29"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void query_resultsWithAnObjectField() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "fql?q=select+app_id%2C+display_name%2C+restriction_info+from+application+where+app_id%3D162886103757745"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void query_resultsWithAnObjectArrayField() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "fql?q=select+uid%2C+name%2C+family+from+user+where+uid%3Dme%28%29"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void nullChecks() {		
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "fql?q=select+stuff+from+somewhere+where+uid%3Dme%28%29"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
