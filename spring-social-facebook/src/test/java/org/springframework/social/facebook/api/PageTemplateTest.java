...
public class PageTemplateTest extends AbstractFacebookApiTest {
	...
	public void getPage_organization() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "140804655931206"))
	...
	public void getPage_product() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "21278871488"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void getPage_place() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "150263434985489"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void getPage_place_with_hours() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "220817147947513"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void getPage_application() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "140372495981006"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void getPage_withExtraData() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "11803178355"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void post_message() throws Exception {
		...
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "987654321/feed"))
	...
	public void postLink() throws Exception {
		...
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "987654321/feed")).andExpect(method(POST))
				.andExpect(header("Authorization", "OAuth someAccessToken"))
				.andExpect(content().string(requestBody))
				.andRespond(withSuccess("{\"id\":\"123456_78901234\"}", MediaType.APPLICATION_JSON));
	...
	@Test
	public void search() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "search?q=coffee&type=place&center=33.050278%2C-96.745833&distance=5280"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	private void expectFetchAccounts() {
		mockServer.expect(requestTo(GraphApi.GRAPH_API_URL + "me/accounts"))
				.andExpect(method(GET))
				.andExpect(header("Authorization", "OAuth someAccessToken"))
